...
function countStatus(records: StoredLeadRecord[], statuses: Set<string>): number { return records.filter((record) => statuses.has(record.lead.pipelineStatus)).length; }
function countStatuses(records: StoredLeadRecord[], statuses: string[]): number { const statusSet = new Set(statuses); return records.filter((record) => statusSet.has(record.lead.pipelineStatus)).length; }
