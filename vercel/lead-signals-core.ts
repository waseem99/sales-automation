  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (sourceKind === 'upwork_email') {
        const upworkEngine = await import('../vercel/upwork-saved-search-engine.js');
        ...
        const linkedInEngine = await import('../vercel/linkedin-warm-signal-engine.js');
        result = await linkedInEngine.processLinkedInWarmSignalBatch({
          state,
          signals: [signal],
          actor,
          generatedAt: now,
