import {
  attachEnrichmentSnapshot,
  buildEvidenceBasedEnrichment,
  enrichmentStateEqual,
  readEnrichmentSnapshot,
  type EnrichmentSnapshot,
} from '@sales-automation/enrichment';
import { loadNeonAppState, persistLeadRecords } from '@sales-automation/neon-state';
import type { StoredLeadRecord } from '@sales-automation/storage';

const ACTOR = 'evidence-enrichment@codistan.local';
const MAX_PROCESSED_IDS = 50;

interface IntakeResponseBody {
  processedLeadIds?: unknown;
  [key: string]: unknown;
}

export interface EnrichmentIntakeSummary {
  status: 'applied';
  processed: number;
  touchedLeadIds: string[];
  companyDomains: number;
  decisionMakers: number;
  verifiedEmails: number;
  candidateEmails: number;
  contactableAfterReview: number;
  suppressed: number;
  openResearchTasks: number;
  providerMode: 'source_visible_only';
  activeProviders: string[];
}

export async function applyEnrichmentAfterIntake(input: {
  response: Response;
  databaseUrl: string;
  generatedAt?: string;
}): Promise<Response> {
  if (!input.response.ok) return input.response;
  const parsed = await parseResponse(input.response);
  if (!parsed) return input.response;
  const processedLeadIds = normalizeIds(parsed.body.processedLeadIds).slice(0, MAX_PROCESSED_IDS);
  if (processedLeadIds.length === 0) return input.response;

  try {
    const generatedAt = input.generatedAt ?? new Date().toISOString();
    const state = await loadNeonAppState(input.databaseUrl);
    const snapshots: EnrichmentSnapshot[] = [];
    const touchedLeadIds: string[] = [];

    for (const leadId of processedLeadIds) {
      const record = state.repository.getLead(leadId);
      if (!record) continue;
      const previous = readEnrichmentSnapshot(record.lead);
      const snapshot = buildEvidenceBasedEnrichment(record.lead, previous, generatedAt);
      snapshots.push(snapshot);
      if (enrichmentStateEqual(previous, snapshot)) continue;
      const enrichedLead = attachEnrichmentSnapshot(record.lead, snapshot);
      state.repository.upsertLead(enrichedLead, ACTOR);
      touchedLeadIds.push(enrichedLead.id);
    }

    const touchedRecords = unique(touchedLeadIds)
      .map((leadId) => state.repository.getLead(leadId))
      .filter((record): record is StoredLeadRecord => Boolean(record));
    await persistLeadRecords(input.databaseUrl, touchedRecords);

    return responseJson({
      ...parsed.body,
      enrichment: buildSummary(snapshots, unique(touchedLeadIds)),
      humanReviewRequired: true,
      externalActionAutomated: false,
    }, input.response.status, parsed.headers);
  } catch (error) {
    console.error('ACQUISITION_ENRICHMENT_POST_INTAKE_ERROR', {
      message: safeErrorMessage(error),
      processedLeadCount: processedLeadIds.length,
    });
    return responseJson({
      ...parsed.body,
      enrichment: {
        status: 'deferred',
        processed: 0,
        providerMode: 'source_visible_only',
        activeProviders: [],
        reason: 'Source ingestion succeeded, but evidence enrichment was deferred for a later retry.',
      },
      humanReviewRequired: true,
      externalActionAutomated: false,
    }, input.response.status, parsed.headers);
  }
}

function buildSummary(snapshots: EnrichmentSnapshot[], touchedLeadIds: string[]): EnrichmentIntakeSummary {
  return {
    status: 'applied',
    processed: snapshots.length,
    touchedLeadIds,
    companyDomains: snapshots.filter((snapshot) => Boolean(snapshot.company.domain)).length,
    decisionMakers: snapshots.filter((snapshot) => Boolean(snapshot.person.name && snapshot.person.role)).length,
    verifiedEmails: snapshots.filter((snapshot) => snapshot.person.email?.verificationStatus === 'verified').length,
    candidateEmails: snapshots.filter((snapshot) => snapshot.person.email?.verificationStatus === 'candidate').length,
    contactableAfterReview: snapshots.filter((snapshot) => snapshot.contactability === 'contactable_after_review').length,
    suppressed: snapshots.filter((snapshot) => snapshot.suppression.suppressed).length,
    openResearchTasks: snapshots.reduce(
      (total, snapshot) => total + snapshot.tasks.filter((task) => task.status === 'open').length,
      0,
    ),
    providerMode: 'source_visible_only',
    activeProviders: [],
  };
}

async function parseResponse(response: Response): Promise<{
  body: IntakeResponseBody;
  headers: Record<string, string>;
} | undefined> {
  try {
    const body = await response.clone().json() as IntakeResponseBody;
    if (!body || typeof body !== 'object' || Array.isArray(body)) return undefined;
    const headers: Record<string, string> = {};
    response.headers.forEach((value, key) => {
      if (key.toLowerCase() !== 'content-length') headers[key] = value;
    });
    return {body, headers};
  } catch {
    return undefined;
  }
}

function normalizeIds(value: unknown): string[] {
  if (!Array.isArray(value)) return [];
  return unique(value.map(String).map((item) => item.trim()).filter(Boolean));
}

function unique(values: string[]): string[] {
  return [...new Set(values)];
}

function safeErrorMessage(error: unknown): string {
  const message = error instanceof Error ? error.message : String(error);
  return message.replace(/postgres(?:ql)?:\/\/[^\s]+/gi, '[database-url-redacted]').slice(0, 500);
}

function responseJson(value: unknown, status: number, existingHeaders: Record<string, string>): Response {
  return new Response(JSON.stringify(value), {
    status,
    headers: {
      ...existingHeaders,
      'content-type': 'application/json; charset=utf-8',
      'cache-control': 'no-store',
    },
  });
}
