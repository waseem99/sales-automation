...
function renderPriorityPage(input: {
  ...
  const counts = {
    priorityA: countBand(input.active, 'priority_a'),
    priorityB: countBand(input.active, 'priority_b'),
    research: countBand(input.active, 'research'),
  ...
  <section class="panel"><div class="section-title"><div><p class="eyebrow">Owner action queue</p><h2>Your top five immediate actions</h2></div><span>${input.topFive.length} actionable</span></div>${input.topFive.length ? input.topFive.map((record, index) => renderPriorityCard(record, index + 1, true)).join('') : '<div class="empty">No uncontacted opportunities meet this threshold. Lower it or improve the missing evidence.</div>'}</section>
  <section class="panel"><div class="section-title"><div><p class="eyebrow">Qualified pipeline</p><h2>All opportunities in scope</h2></div><span>${input.active.length} records</span></div>${input.active.length ? input.active.map((record, index) => renderPriorityCard(record, index + 1, false)).join('') : '<div class="empty">No opportunities meet this threshold.</div>'}</section>
