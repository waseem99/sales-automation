...
function renderWorkspace(input: { actor: string; recent: ReturnType<typeof recentLinkedInSignals> }): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>LinkedIn Warm Signals</title><style>${styles()}</style></head><body><main>
  <header><div><p class="eyebrow">Compliant warm-signal intake</p><h1>LinkedIn & Sales Navigator Signals</h1><p>Paste a buyer-side LinkedIn post or Sales Navigator alert. The system rejects jobs, self-promotion and stale content, then deduplicates, enriches, assigns and adds qualified records to the Priority queue.</p></div><nav><a href="/priorities">Priorities</a><a href="/operations">Operations</a><a href="/prospects">Prospects</a></nav></header>
  <section class="notice"><strong>No logged-in LinkedIn crawling.</strong> No connections, InMails, comments or applications are sent automatically. Public-index snippets remain research-only until the original post is opened and verified.</section>
  <section class="panel"><h2>Add a signal</h2><form method="post" action="/api/linkedin-signals">
  ...
  <section class="panel"><div class="title"><h2>Recent LinkedIn signals</h2><span>${input.recent.length}</span></div>${renderRecent(input.recent)}</section>
...
function renderRecent(items: ReturnType<typeof recentLinkedInSignals>): string {
  if (!items.length) return '<p>No LinkedIn warm signals have been stored yet.</p>';
...
function styles(): string { return `:root{font-family:Inter,ui-sans-serif,system-ui;color:#172033;background:#f4f6fb}*{box-sizing:border-box}body{margin:0}main{max-width:1200px;margin:32px auto;padding:0 20px}header{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.eyebrow{text-transform:uppercase;letter-spacing:.08em;font-size:11px;font-weight:800;color:#667085}h1,h2{margin:.2rem 0}p,footer{color:#667085}nav{display:flex;gap:12px;flex-wrap:wrap}a{color:#3157d5}.notice,.panel{background:#fff;border:1px solid #e4e7ec;border-radius:14px;padding:18px;margin:16px 0}.notice{border-color:#b9c8f5;background:#f5f7ff}form{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:18px}label{display:grid;gap:6px;font-size:12px;font-weight:750}.wide{grid-column:1/-1}input,textarea,select{border:1px solid #d0d5dd;border-radius:9px;padding:10px;font:inherit}button{border:0;border-radius:9px;background:#3157d5;color:#fff;padding:12px;font:inherit;font-weight:800;cursor:pointer}.title{display:flex;justify-content:space-between}.table{overflow:auto}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:10px;border-bottom:1px solid #eef0f3;font-size:12px;vertical-align:top}td small{display:block;color:#667085;margin-top:4px}.metrics{display:grid;grid-template-columns:repeat(6,1fr);gap:10px}.metrics article{background:#fff;border:1px solid #e4e7ec;border-radius:11px;padding:14px}.metrics strong{display:block;font-size:24px}.metrics span{font-size:11px;color:#667085}pre{white-space:pre-wrap;background:#fff;border:1px solid #e4e7ec;border-radius:10px;padding:12px}@media(max-width:760px){header{display:block}form{grid-template-columns:1fr}.wide{grid-column:auto}.metrics{grid-template-columns:repeat(2,1fr)}}`; }
