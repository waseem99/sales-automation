...
const readiness = fs.readFileSync(path.join(acquisition, 'scripts/windows/check-sales-automation-release.ps1'), 'utf8');
const pilot = fs.readFileSync(path.join(acquisition, 'scripts/windows/check-sales-automation-pilot.ps1'), 'utf8');
const pilotModel = fs.readFileSync(path.join(acquisition, 'acquisition_v4/sales_automation_acceptance.py'), 'utf8');
const commercial = fs.readFileSync(path.join(root, 'packages/commercial-readiness/src/index.ts'), 'utf8');
const canonicalInstall = fs.readFileSync(path.join(acquisition, 'START-HERE-SALES-AUTOMATION.cmd'), 'utf8');
const canonicalStart = fs.readFileSync(path.join(acquisition, 'START-SALES-AUTOMATION.cmd'), 'utf8');
const canonicalCheck = fs.readFileSync(path.join(acquisition, 'CHECK-SALES-AUTOMATION-RELEASE.cmd'), 'utf8');
const canonicalPilot = fs.readFileSync(path.join(acquisition, 'CHECK-SALES-AUTOMATION-PILOT.cmd'), 'utf8');
const docs = fs.readFileSync(path.join(root, 'docs/sales-automation/RELEASE-1.0-RC1.md'), 'utf8');
...
for (const marker of [
  ...
  'Start Sales Automation.lnk', 'Check Sales Automation Release.lnk', 'Check Sales Automation Pilot.lnk',
...
for (const marker of [
  ...
  'codistan-sales-automation-acceptance.v1',
...
assert(canonicalInstall.includes('install-acquisition-v4.ps1'));
assert(canonicalStart.includes('START-ACQUISITION-V4.cmd'));
assert(canonicalCheck.includes('check-sales-automation-release.ps1'));
assert(canonicalPilot.includes('check-sales-automation-pilot.ps1'));
...
assert(docs.includes('CHECK-SALES-AUTOMATION-PILOT.cmd'));
...
console.log('Sales Automation release, commercial readiness and pilot acceptance contract passed.');
