...
from acquisition_v4.prospecting_os_acceptance import build_report
from acquisition_v4.storage import AtomicRecordStore, atomic_write_text


class TalentTrackAcceptanceTests(unittest.TestCase):
    ...
    def _reviews(self, root: Path, accepted: int = 9) -> None:
        ...
        for index, identity in enumerate(identities):
            reviews.append({
                ...
                "reviewed_at": "2026-07-28T08:00:00.000Z",
                "notes": "Commercial relevance reviewed manually.",
            })
        atomic_write_text(
            root / "review" / "commercial-review.json",
            json.dumps({
                "schema_version": "codistan-prospecting-os-commercial-review.v1",
                "reviews": reviews,
            }, indent=2) + "\n",
        )
