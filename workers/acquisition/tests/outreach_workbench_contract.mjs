...
for (const marker of ['coldFintech', 'approvalAllowed, false', 'coldPartner', 'approvalAllowed, true', 'warmFintech']) {
  assert(readinessTests.includes(marker), `missing commercial readiness test: ${marker}`);
}

for (const marker of [
  'Authentication required', 'visibility(actor)', 'persistLeadRecords', 'recordBdPipelineEvent',
  'externalActionPerformedBySystem: false', 'evaluateCommercialReadiness',
  'assertCommerciallyReadyForApproval(record.lead)', 'commercialReadiness', 'commercial approval blocked',
]) assert(api.includes(marker), `missing authenticated API marker: ${marker}`);

for (const marker of [
  'Outreach review workbench', 'Import generated drafts', 'Approve exact revision', 'Confirm manually sent',
  'It never sends, submits', 'data-outreach-workbench-script', 'Commercial readiness',
  'Approval blockers', 'Prohibited or unsupported claims', 'Proof limitations', 'Approval blocked',
