...
for (const marker of ['Authentication required', 'visibility(actor)', 'persistLeadRecords', 'recordBdPipelineEvent', 'externalActionPerformedBySystem: false']) {
  assert(api.includes(marker), `missing authenticated API marker: ${marker}`);
}

for (const marker of ['Outreach review workbench', 'Import generated drafts', 'Approve exact revision', 'Confirm manually sent', 'It never sends, submits', 'data-outreach-workbench-script']) {
...
console.log('Outreach workbench contract passed.');
