...
function Find-Python {
    ...
    throw "Python 3.12 or later is required. Run START-HERE-TALENTTRACK.cmd first."
}
...
foreach ($entry in $collectorPorts.GetEnumerator()) {
    try {
        ...
        Write-Host "$($entry.Key): HEALTHY (runtime $($health.runtime_version), storage $($health.storage_backend))"
    } catch {
        Write-Host "$($entry.Key): UNAVAILABLE OR UNSAFE" -ForegroundColor Red
        Write-Host "Run START-TALENTTRACK.cmd, then rerun this checker."
        exit 3
    }
}
...
if (-not (Test-Path (Join-Path $packageRoot "acquisition_v4\prospecting_os_acceptance.py"))) {
    throw "The TalentTrack combined acceptance module is missing from the installed package."
}
...
Write-Host ""
Write-Host "Combined report: $(Join-Path $reviewDirectory 'talenttrack-pilot-acceptance.json')"
