...
if (-not $manifest) {
    ...
    $manifest = [pscustomobject]@{
        product = "Codistan Prospecting OS"
...
$commercialReady = $localReady -and $syncConfigured

$report = [ordered]@{
    schema_version = "codistan-prospecting-os-readiness.v1"
...
$reportPath = Join-Path $reviewDirectory "prospecting-os-release-readiness.json"
$report | ConvertTo-Json -Depth 12 | Set-Content -Path $reportPath -Encoding UTF8

Write-Host ""
Write-Host "$($report.product) $($report.release_version)"
