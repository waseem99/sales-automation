...
function Find-Python {
    ...
    throw "Python 3.12 or later is required. Run START-HERE-SALES-AUTOMATION.cmd first."
}
...
foreach ($entry in $collectorPorts.GetEnumerator()) {
    try {
        ...
        Write-Host "Run Start Sales Automation, then rerun this checker."
        exit 3
    }
}

$packageRoot = Join-Path $InstallRoot "workers\acquisition"
if (-not (Test-Path (Join-Path $packageRoot "acquisition_v4\sales_automation_acceptance.py"))) {
    throw "The Sales Automation acceptance module is missing from the installed package."
}
...
& $pythonExe @pythonArgs -m acquisition_v4.sales_automation_acceptance --state-root $StateRoot
$exitCode = $LASTEXITCODE
Write-Host ""
Write-Host "Combined report: $(Join-Path $reviewDirectory 'sales-automation-pilot-acceptance.json')"
