...
REVIEW_SCHEMA = "codistan-sales-automation-commercial-review.v1"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Evaluate the complete Sales Automation live pilot and human commercial gate.")
    ...
    output_path = args.state_root / "review" / "sales-automation-pilot-acceptance.json"
    atomic_write_text(output_path, json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n")

    if args.as_json:
        print(json.dumps(report, indent=2, sort_keys=True))
    else:
        print(f"SALES AUTOMATION PILOT: {report['status'].upper()}")
