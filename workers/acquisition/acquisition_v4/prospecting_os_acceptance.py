...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Evaluate the complete TalentTrack live pilot and human commercial gate.")
    ...
    report["schema_version"] = "codistan-talenttrack-pilot-acceptance.v1"
    output_path = args.state_root / "review" / "talenttrack-pilot-acceptance.json"
    atomic_write_text(output_path, json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n")

    if args.as_json:
        print(json.dumps(report, indent=2, sort_keys=True))
    else:
        print(f"TALENTTRACK PILOT: {report['status'].upper()}")
