...
import { loadDiscoverySourceControls, sourceControlMap } from '@sales-automation/neon-state/source-controls';
import {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      const inbox = upworkEnabled || linkedinInboxEnabled
        ...
        }).catch((error) => emptyInboxResult(inboxConfig.configured, errorMessage(error)))
        : emptyInboxResult(inboxConfig.configured);
...
function emptyInboxResult(configured: boolean, error?: string) {
  return {
    configured,
    checked: 0,
    accepted: 0,
