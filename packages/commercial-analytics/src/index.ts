...
function buildDimensionLanes(
  ...
  for (const fact of facts) {
    for (const lane of fact.dimensions[dimension]) {
      const existing = groups.get(lane.key) ?? {label: lane.label, facts: []};
      existing.facts.push(fact);
      groups.set(lane.key, existing);
    }
  }
...
function inspectRecord(record: StoredLeadRecord, period: CommercialPeriod): RecordFacts {
  ...
  const sentRecords = workbench?.drafts.flatMap((draft) => draft.sentHistory.map((sent) => ({...sent, draftChannel: draft.channel}))) ?? [];
  const outcomes = workbench?.drafts.flatMap((draft) => draft.outcomes) ?? [];
  ...
  const replyAt = earliestIso([
    ...outcomes.filter((item) => item.outcome === 'reply').map((item) => item.occurredAt),
    lead.lastResponseAt,
    statusAt(statusEvents, 'replied'),
  ]);
  const meetingAt = earliestIso([
    ...outcomes.filter((item) => item.outcome === 'meeting').map((item) => item.occurredAt),
    statusAt(statusEvents, 'meeting_booked'),
  ]);
  const proposalAt = earliestIso([
    ...outcomes.filter((item) => item.outcome === 'proposal').map((item) => item.occurredAt),
    statusAt(statusEvents, 'proposal_sent'),
  ]);
  ...
  const dueFollowUps = sentRecords.flatMap((sent) => sent.followUpAt ? [{
    dueAt: sent.followUpAt,
    completedAt: earliestAfter([
      ...sentRecords.map((item) => item.sentAt),
      replyAt,
      meetingAt,
      proposalAt,
    ], sent.followUpAt),
  ...
  return {
    ...
    dimensions: dimensionsFor(record, sentRecords.map((item) => item.draftChannel), workbench?.recommendedChannel),
  };
}

function dimensionsFor(record: StoredLeadRecord, sentChannels: string[], recommendedChannel?: string): Record<CommercialDimension, Array<{key: string; label: string}>> {
  const lead = record.lead;
  const campaign = campaignLane(lead);
  const channels = uniqueStrings(sentChannels.length ? sentChannels : [recommendedChannel ?? channelFallback(lead)]);
  return {
...
function aiDispositionFor(workbench: ReturnType<typeof readOutreachWorkbench>): AiRecommendationDisposition {
  ...
  const approved = drafts.find((draft) => draft.approvedVersionId);
  if (!approved?.approvedVersionId) return 'pending';
  const version = approved.versions.find((item) => item.id === approved.approvedVersionId);
  return version?.origin === 'human_edit' ? 'edited' : 'accepted';
}
...
function channelFallback(lead: Lead): string {
  if (lead.source === 'upwork') return 'upwork_proposal';
  if (lead.source === 'sales_navigator') return 'sales_navigator_inmail';
  ...
  if (/referral|introduction/i.test(lead.reachMethod ?? '')) return 'referral';
  return 'unrecorded';
}
...
function earliestAfter(values: Array<string | undefined>, after: string): string | undefined {
  const threshold = Date.parse(after);
  return values.flatMap((value) => validIso(value) ? [validIso(value)!] : []).filter((value) => Date.parse(value) >= threshold).sort()[0];
}
