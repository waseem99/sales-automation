...
export function renderPaginatedProspectDashboardPage(input: PaginatedProspectDashboardInput): string {
  ...
  const topActions = access.canRunGlobalOperations
    ? `<div class="top-actions"><button id="manual-intake-open" class="ghost">Add approved opportunity</button><button id="pseb-sync" class="ghost">Sync PSEB collection</button><button id="import-starter" class="ghost">Load verified prospects</button><button id="assign-owners" class="ghost">Assign unassigned</button><button id="refresh-recent" class="primary">Refresh last 78 hours</button></div>`
  ...
  const intakeDialog = access.canRunGlobalOperations ? renderManualIntakeDialog() : '';
  html = html.replace('</script></body>', `</script>${intakeDialog}<script>${paginationScript()}</script></body>`);
  return html;
}
...
function renderManualIntakeDialog(): string {
  return `<dialog id="manual-intake-dialog" class="manual-intake-dialog"><form id="manual-intake-form" method="dialog"><div class="manual-intake-heading"><div><span>Approved source intake</span><h2>Add opportunity to Prospect Desk</h2><p>Paste team-supplied public evidence or referral context. The system will normalize, deduplicate, score, assign and prepare guidance, but it will not contact anyone or submit anything externally.</p></div><button id="manual-intake-close" type="button" class="dialog-close" aria-label="Close">×</button></div><div class="manual-intake-grid"><label>Source type<select name="sourceKind" required><option value="auto_batch">Auto-detect / batch</option><option value="upwork_email">Upwork alert or email</option><option value="linkedin_signal">LinkedIn signal</option><option value="sales_navigator_alert">Sales Navigator alert</option><option value="public_post">Public post</option><option value="public_url">Public opportunity URL</option><option value="referral_note">Referral note</option><option value="copied_alert">Other copied alert</option></select></label><label>Public source URL<input name="sourceUrl" type="url" placeholder="https://… (required for Public URL)" /></label><label>Title override<input name="title" placeholder="Optional opportunity title" /></label><label>Company<input name="companyName" placeholder="Optional company or buyer" /></label><label>Contact name<input name="contactName" placeholder="Optional public contact" /></label><label>Contact role<input name="contactRole" placeholder="Optional role" /></label><label>Country<input name="country" placeholder="Optional country" /></label><label>Region<input name="region" placeholder="Optional city, province or region" /></label><label class="manual-intake-content">Opportunity evidence or notes<textarea name="content" rows="10" minlength="20" required placeholder="Paste the complete alert, public post, opportunity description, Sales Navigator text, referral note, or multiple blocks separated by ---"></textarea></label></div><div class="manual-intake-footer"><p id="manual-intake-status" aria-live="polite">Human review remains mandatory before outreach, bidding or submission.</p><div><button id="manual-intake-cancel" type="button" class="ghost">Cancel</button><button type="submit" class="primary">Qualify and save</button></div></div></form></dialog>`;
}
...
const manualDialog=document.getElementById('manual-intake-dialog'),manualForm=document.getElementById('manual-intake-form'),manualStatus=document.getElementById('manual-intake-status');
function closeManualIntake(){if(manualDialog?.open)manualDialog.close();}
document.getElementById('manual-intake-open')?.addEventListener('click',()=>manualDialog?.showModal());
document.getElementById('manual-intake-close')?.addEventListener('click',closeManualIntake);
document.getElementById('manual-intake-cancel')?.addEventListener('click',closeManualIntake);
manualDialog?.addEventListener('click',event=>{if(event.target===manualDialog)closeManualIntake();});
manualForm?.addEventListener('submit',async event=>{event.preventDefault();const button=manualForm.querySelector('button[type=submit]'),original=button?.textContent||'Qualify and save';if(button){button.disabled=true;button.textContent='Qualifying…';}manualStatus.textContent='Normalizing evidence, checking duplicates, scoring, assigning and preparing human-reviewed guidance…';const payload=Object.fromEntries(new FormData(manualForm).entries());try{const response=await fetch('/api/prospects/manual-intake',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await response.json();if(!response.ok)throw new Error(data.error||'Manual intake failed');manualStatus.textContent=data.created?(data.created+' opportunity saved; '+data.duplicates+' duplicate(s) skipped. Opening the new record…'):('No new opportunity saved; '+data.duplicates+' duplicate(s) matched existing records.');if(data.prospectUrl)setTimeout(()=>location.href=data.prospectUrl,700);else setTimeout(()=>location.reload(),900);}catch(error){manualStatus.textContent=error.message;if(button){button.disabled=false;button.textContent=original;}}});
