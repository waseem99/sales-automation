...
async function main(): Promise<void> {
  ...
  const login = async (password: string): Promise<Response> => handler.fetch(new Request(
    'https://example.test/api/dashboard?__path=/api/login',
    {
      method: 'POST',
      headers: {
  ...
  const payload = await validAfterLockout.json() as {
    identifier?: string;
    access?: { scope?: string };
  };
